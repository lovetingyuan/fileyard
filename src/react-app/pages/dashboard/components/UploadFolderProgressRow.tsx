...
function getFolderStatusLabel(row: UploadFolderProgressDisplayRow): string {
  const segments = [`${row.remaining} 个进行中`];
  if (row.failed > 0) {
    segments.push(`${row.failed} 个失败`);
  }
  if (row.completed > 0) {
    segments.push(`${row.completed} 个完成`);
  }
  if (row.canceled > 0) {
    ...
    <li className="rounded-box bg-base-200/70 px-3 py-2">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-xs text-base-content/60" title={getFolderStatusLabel(row)}>
            {getFolderStatusLabel(row)}
          </p>
        </div>
        <span className="badge badge-xs shrink-0 border-cyan-200 bg-cyan-100 text-cyan-800">
          上传中
        </span>
